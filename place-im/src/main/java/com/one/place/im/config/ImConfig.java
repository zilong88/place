...
import com.one.place.im.core.ImHandler;
import com.one.place.im.core.cluster.ImCluster;
import com.one.place.im.core.listener.ImGroupListener;
import com.one.place.im.core.listener.ImGroupListenerAdapter;
import com.one.place.im.core.listener.ImListener;
import com.one.place.im.core.listener.ImUserListener;
...
public abstract class ImConfig extends MapWithLockPropSupport implements ImConst {
   ...
   * J-IM内部线程池
   */
  protected SynThreadPoolExecutor imExecutor;
  /**
   * 群组绑定监听器
   */
  ...
  public ImConfig(SynThreadPoolExecutor imExecutor){
    this.imExecutor = imExecutor;
    if (this.imExecutor == null) {
      LinkedBlockingQueue<Runnable> timQueue = new LinkedBlockingQueue<>();
      this.imExecutor = new SynThreadPoolExecutor(CORE_POOL_SIZE, CORE_POOL_SIZE, Threads.KEEP_ALIVE_TIME, timQueue,
          DefaultThreadFactory.getInstance(ImConst.IM, Thread.NORM_PRIORITY), ImConst.IM);
      this.imExecutor.prestartAllCoreThreads();
    }
  }
  ...
  public SynThreadPoolExecutor getImExecutor() {
    return imExecutor;
  }
